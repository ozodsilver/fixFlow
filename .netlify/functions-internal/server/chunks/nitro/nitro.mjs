import process from 'node:process';globalThis._importMeta_=globalThis._importMeta_||{url:"file:///_entry.js",env:process.env};globalThis.__timing__.logStart('Load chunks/nitro/nitro');import { createHash, timingSafeEqual, createHmac } from 'node:crypto';
...
const _inlineRuntimeConfig = {
  "app": {
    "baseURL": "/",
    "buildId": "9d609dfc-e9bc-4931-944e-73a853743d77",
  ...
  "public": {
    "supabaseUrl": "https://kpykryshqykwustuplol.supabase.co",
    "allowDevAuthBypass": false
  },
  "sessionSecret": "qDmwVY4S8AIxngaiiiroFILL40hsfgZuWzA8dwUHKNI/VO4n7gZAQj6YYX2RL/y/",
  "adminSessionSecret": "H134xCyay9yDpTP3DcM4x4jT+J0bAX/BdnUr89uMdEd1mYy68KBJoHtn49UjG4rh",
  "adminLogin": "admin",
  "adminPassword": "admin123",
  "telegramBotToken": "8674773445:AAH5awm39WLuHADGWeRf4_KEIKeGfeuIbXk",
  "telegramWebhookSecret": "",
  "telegramAdminChatId": "-838438395",
  "telegramMastersGroupId": "-1003924099561",
  "miniAppBaseUrl": "https://fixflowchat.netlify.app/",
  "telegramBotUsername": "fixflowchatbot",
  "telegramMiniAppShortName": "hgs",
  "supabaseServiceRoleKey": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtweWtyeXNocXlrd3VzdHVwbG9sIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3Njk0NjcyMCwiZXhwIjoyMDkyNTIyNzIwfQ.cPIm12JBhHVPJlftzwh3qiVoZ3xglobhn9kaLay8fFo",
  "geminiApiKey": "AIzaSyBgKJ2m8OGQ1BVQW_o9lr8Grc-rvJxGhjQ",
...
const _ZKWUFsXjz8baYfGRDMwWE9t90CnPLscvXsBEKMhadQ = (function(nitro) {
...
const _9I4tcuCO6dhHsg7KXqud8LGoqOWT6zDxUIRlAXDIjm4 = defineNitroPlugin((nitro) => {
...
const _nWvdbJI2bE3Dv3wytdvjlxU6ybu2sISoVLrmw6BEmq8 = defineNitroPlugin((nitro) => {
...
const plugins = [
  _ZKWUFsXjz8baYfGRDMwWE9t90CnPLscvXsBEKMhadQ,
_9I4tcuCO6dhHsg7KXqud8LGoqOWT6zDxUIRlAXDIjm4,
_nWvdbJI2bE3Dv3wytdvjlxU6ybu2sISoVLrmw6BEmq8
];
...
function toBase64Url$1(value) {
  return Buffer.from(value).toString("base64url");
}
function signPayload$1(encodedPayload, secret) {
  return toBase64Url$1(createHmac("sha256", secret).update(encodedPayload).digest());
}
function createAdminSessionToken(payload, secret) {
  const encodedPayload = toBase64Url$1(JSON.stringify(payload));
  const signature = signPayload$1(encodedPayload, secret);
  return `${encodedPayload}.${signature}`;
}
function verifyAdminSessionToken(token, secret) {
  const [encodedPayload, signature] = token.split(".");
  if (!encodedPayload || !signature) return null;
  const expected = signPayload$1(encodedPayload, secret);
...
function getSupabaseAdmin(event) {
  const config = useRuntimeConfig(event);
...
function setSessionCookie(event, userId, telegramUserId) {
  const config = useRuntimeConfig(event);
...
async function requireUserContext(event) {
  const session = getSessionPayload(event);
  const supabase = getSupabaseAdmin(event);
  const { data: user, error: userError } = await supabase.from("users").select("id, telegram_user_id, display_name, username, locale, is_blocked").eq("id", session.uid).single();
  if (userError || !user) {
    apiError(401, "auth.session_expired", "Session expired");
  }
  if (user.is_blocked) {
...
async function requireOwnedRequest(event, requestId) {
...
function visitTimeLabel(mode, at, locale) {
  if (mode === "asap") return locale === "ru" ? "\u0421\u0440\u043E\u0447\u043D\u043E (ASAP)" : "\u0428\u043E\u0448\u0438\u043B\u0438\u043D\u0447 (ASAP)";
  if (mode === "scheduled" && at) {
    const date = new Date(at);
    const formatted = Number.isNaN(date.getTime()) ? at : new Intl.DateTimeFormat(locale === "ru" ? "ru-RU" : "uz-Cyrl-UZ", {
      year: "numeric",
      month: "2-digit",
      day: "2-digit",
      hour: "2-digit",
...
async function getDomainName(event, domainId, locale) {
  const supabase = getSupabaseAdmin(event);
  const { data } = await supabase.from("service_domains").select("name_uz_cyrl, name_ru").eq("id", domainId).maybeSingle();
  if (!data) return `#${domainId}`;
  return locale === "ru" ? data.name_ru : data.name_uz_cyrl;
}
async function buildDispatchPreviewText(event, input) {
  ...
  const privacyNote = input.locale === "ru" ? "\u0422\u0435\u043B\u0435\u0444\u043E\u043D \u0438 \u0442\u043E\u0447\u043D\u044B\u0439 \u0430\u0434\u0440\u0435\u0441 \u043E\u0442\u043A\u0440\u043E\u044E\u0442\u0441\u044F \u0442\u043E\u043B\u044C\u043A\u043E \u043F\u043E\u0441\u043B\u0435 \u0443\u0441\u043F\u0435\u0448\u043D\u043E\u0433\u043E \u043F\u0440\u0438\u043D\u044F\u0442\u0438\u044F \u0437\u0430\u043A\u0430\u0437\u0430." : "\u0422\u0435\u043B\u0435\u0444\u043E\u043D \u0432\u0430 \u0430\u043D\u0438\u049B \u043C\u0430\u043D\u0437\u0438\u043B \u0431\u0443\u044E\u0440\u0442\u043C\u0430 \u043C\u0443\u0432\u0430\u0444\u0444\u0430\u049B\u0438\u044F\u0442\u043B\u0438 \u049B\u0430\u0431\u0443\u043B \u049B\u0438\u043B\u0438\u043D\u0433\u0430\u043D\u0434\u0430\u043D \u043A\u0435\u0439\u0438\u043D \u043E\u0447\u0438\u043B\u0430\u0434\u0438.";
  return [
    `\u{1F4CC} ${intro}: ${input.public_code}`,
    `\u{1F6E0}\uFE0F ${input.locale === "ru" ? "\u0423\u0441\u043B\u0443\u0433\u0430" : "\u0425\u0438\u0437\u043C\u0430\u0442"}: ${domainName}`,
    `\u{1F4DD} ${summaryTitle}: ${input.problem_summary || (input.locale === "ru" ? "\u041D\u0435 \u0443\u043A\u0430\u0437\u0430\u043D\u043E" : "\u041A\u045E\u0440\u0441\u0430\u0442\u0438\u043B\u043C\u0430\u0433\u0430\u043D")}`,
    `\u23F0 ${timeTitle}: ${visitTimeLabel(input.visit_time_mode, input.visit_time_at, input.locale)}`,
    `\u{1F512} ${privacyNote}`
  ].join("\n");
}
function buildAdminReviewNotificationText(input) {
  ...
  if (input.locale === "ru") {
    return [
      `\u{1F195} \u041D\u043E\u0432\u0430\u044F \u0437\u0430\u044F\u0432\u043A\u0430 \u043E\u0436\u0438\u0434\u0430\u0435\u0442 \u043F\u0440\u043E\u0432\u0435\u0440\u043A\u0438: ${input.public_code}`,
  ...
  return [
    `\u{1F195} \u042F\u043D\u0433\u0438 \u043C\u0443\u0440\u043E\u0436\u0430\u0430\u0442 \u0442\u0435\u043A\u0448\u0438\u0440\u0443\u0432 \u043A\u0443\u0442\u044F\u043F\u0442\u0438: ${input.public_code}`,
...
function buildMasterClaimNotificationText(input) {
  const problem = input.problem_summary || (input.locale === "ru" ? "\u041D\u0435 \u0443\u043A\u0430\u0437\u0430\u043D\u043E" : "\u041A\u045E\u0440\u0441\u0430\u0442\u0438\u043B\u043C\u0430\u0433\u0430\u043D");
  const phone = input.phone_e164 || (input.locale === "ru" ? "\u041D\u0435 \u0443\u043A\u0430\u0437\u0430\u043D\u043E" : "\u041A\u045E\u0440\u0441\u0430\u0442\u0438\u043B\u043C\u0430\u0433\u0430\u043D");
...
async function sendTelegramUserMessage(botToken, telegramUserId, text, button) {
  var _a, _b;
  try {
    const payloadBody = {
    ...
    if ((button == null ? void 0 : button.text) && button.url) {
    ...
    if (!response.ok || !(payload == null ? void 0 : payload.ok)) {
      return { ok: false, messageId: null, error: (payload == null ? void 0 : payload.description) || `HTTP ${response.status}` };
    }
    return { ok: true, messageId: (_b = (_a = payload.result) == null ? void 0 : _a.message_id) != null ? _b : null };
  } catch (error) {
    return {
      ok: false,
      messageId: null,
...
async function sendTelegramDispatchMessage(botToken, groupId, text, button) {
  const sendOnce = async (inlineButton) => {
    var _a, _b;
    const payloadBody = {
      chat_id: groupId,
      text
  ...
  if (!(button == null ? void 0 : button.text) || !button.webAppUrl && !button.url) {
    const sent = await sendOnce();
    return sent.ok ? { ok: true, messageId: sent.messageId } : { ok: false, messageId: null, error: sent.error };
  }
  if (button.webAppUrl) {
    ...
    if (button.url) {
      const urlSent2 = await sendOnce({ text: button.text, url: button.url });
      if (urlSent2.ok) {
        return { ok: true, messageId: urlSent2.messageId };
      }
      return { ok: false, messageId: null, error: `${webAppSent.error}; fallback failed: ${urlSent2.error}` };
    }
    return { ok: false, messageId: null, error: webAppSent.error };
  }
  const urlSent = await sendOnce({ text: button.text, url: button.url });
  return urlSent.ok ? { ok: true, messageId: urlSent.messageId } : { ok: false, messageId: null, error: urlSent.error };
}

function buildClaimButton(config, dispatchId) {
  ...
  const tMeDirectUrl = botUsername && miniAppShortName ? `https://t.me/${botUsername}/${miniAppShortName}?startapp=${startApp}` : void 0;
  const tMeMainUrl = botUsername ? `https://t.me/${botUsername}?startapp=${startApp}` : void 0;
...
async function createAndSendDispatch(event, request) {
  ...
  const { data: dispatch, error: dispatchError } = await supabase.from("dispatch_records").insert({
  ...
  if (dispatchError || !dispatch) {
    apiError(500, "db.failed", "Failed to create dispatch record", { reason: dispatchError == null ? void 0 : dispatchError.message });
  }
  ...
  const sent = await sendTelegramDispatchMessage(
    ...
    previewText,
    claimButton ? { text: claimButtonText, ...claimButton } : void 0
  );
  if (!sent.ok) {
  ...
  await supabase.from("dispatch_records").update({
    status: "open",
...
async function createPendingDispatchReview(event, request) {
  const supabase = getSupabaseAdmin(event);
  const { data: existing, error: lookupError } = await supabase.from("admin_dispatch_reviews").select("id").eq("request_id", request.id).eq("status", "pending").maybeSingle();
  if (lookupError) {
    apiError(500, "db.failed", "Failed to lookup admin dispatch review", { reason: lookupError.message });
  }
  if (existing) return existing.id;
  const { data: review, error: reviewError } = await supabase.from("admin_dispatch_reviews").insert({
    request_id: request.id,
    requester_id: request.requester_id
  }).select("id").single();
  if (reviewError || !review) {
    apiError(500, "db.failed", "Failed to create admin dispatch review", { reason: reviewError == null ? void 0 : reviewError.message });
  }
  return review.id;
}
...
function normalizeIssueCustom(raw) {
  const value = raw.trim().toLowerCase().replace(/[^\p{L}\p{N}_\- ]/gu, "").replace(/\s+/g, "_").slice(0, 64);
  return value || null;
}
function normalizeVisitTimeAt(raw) {
...
function normalizeVisitMode(raw) {
  if (typeof raw !== "string") return null;
  if (raw === "asap" || raw === "scheduled") return raw;
  return null;
}
function asShortText(raw, max) {
  if (typeof raw !== "string") return null;
  const value = raw.trim().replace(/\s+/g, " ").slice(0, max);
  return value || null;
}
function computeMissingFields(current) {
  var _a, _b, _c;
  const missing = [];
  if (!((_a = current.problem_summary) == null ? void 0 : _a.trim())) {
    missing.push("problem_summary");
  }
  if (!((_b = current.phone_e164) == null ? void 0 : _b.trim())) {
    missing.push("phone");
  }
  if (!((_c = current.address_text) == null ? void 0 : _c.trim()) || current.address_lat === null || current.address_lng === null) {
    missing.push("address");
  }
  return missing;
}
function buildPrompt(locale, missing) {
  if (missing.length === 0) {
    return locale === "ru" ? "\u0421\u043F\u0430\u0441\u0438\u0431\u043E, \u0434\u0430\u043D\u043D\u044B\u0435 \u043F\u043E\u043B\u0443\u0447\u0438\u043B\u0438. \u0412\u0430\u0448\u0438 \u0434\u0430\u043D\u043D\u044B\u0435 \u043E\u0442\u043F\u0440\u0430\u0432\u043B\u0435\u043D\u044B \u043C\u0430\u0441\u0442\u0435\u0440\u0430\u043C." : "\u0420\u0430\u04B3\u043C\u0430\u0442, \u043C\u0430\u044A\u043B\u0443\u043C\u043E\u0442\u043B\u0430\u0440\u043D\u0438 \u043E\u043B\u0434\u0438\u043A. \u041C\u0430\u044A\u043B\u0443\u043C\u043E\u0442\u043B\u0430\u0440\u0438\u043D\u0433\u0438\u0437 \u043C\u0430\u0441\u0442\u0435\u0440\u043B\u0430\u0440\u0433\u0430 \u044E\u0431\u043E\u0440\u0438\u043B\u0434\u0438.";
  }
...
function fallbackAnalyze(input) {
  ...
  if (!input.current.address_text && (/(кўча|ko['’]?cha|улица|дом|uy|уй|manzil|манзил|адрес|mahalla|mahallasi|mikrorayon|микрорайон)/i.test(text) || initialMissing.includes("address") && text.length >= 6)) {
...
function sanitizeUpdates(updates, current) {
  var _a, _b, _c;
  if (!updates || typeof updates !== "object") {
    return {};
  }
  const normalized = {};
  const issueCustomRaw = asShortText((_b = (_a = updates.issue_custom) != null ? _a : updates.issue_type) != null ? _b : updates.problem_type, 64);
  if (issueCustomRaw) {
  ...
  const phoneRaw = asShortText((_c = updates.phone_raw) != null ? _c : updates.phone_e164, 32);
  if (phoneRaw) {
  ...
  if (normalized.consent_share === void 0 && !current.consent_share && typeof updates.consent_text === "string" && /(розиман|согласен|разрешаю|consent|ok)/i.test(updates.consent_text)) {
    normalized.consent_share = true;
  }
  return normalized;
}
function toSafeIntent(raw, fallback) {
...
async function callGemini(input, options) {
  var _a, _b, _c, _d, _e;
  const apiKey = (_a = options.geminiApiKey) == null ? void 0 : _a.trim();
  ...
  const payload = await response.json();
  const content = ((_e = (_d = (_c = (_b = payload.candidates) == null ? void 0 : _b[0]) == null ? void 0 : _c.content) == null ? void 0 : _d.parts) == null ? void 0 : _e.map((part) => part.text || "").join("\n")) || "";
  const parsed = extractJsonObject(content);
  if (!parsed) return null;
  return parsed;
}
function normalizeAiReply(raw, locale, missing) {
...
async function analyzeIntakeMessage(input, options = {}) {
  const text = input.text.trim();
  if (!text) {
    return {
      intent: "collect",
  ...
  const missing = computeMissingFields(merged);
  const intent = missing.length === 0 ? geminiIntent === "confirm" ? "confirm" : "ready" : "collect";
  return {
    intent,
...
async function requireMasterContext(event) {
  const config = useRuntimeConfig(event);
  const supabase = getSupabaseAdmin(event);
...
const _WYTxYx = defineCachedEventHandler(async (event) => {
...
const _28eN2n = lazyEventHandler(() => {
...
const _lazy_DK8o7k = () => import('../routes/api/v1/admin/auth/login.post.mjs');
const _lazy_vmb_cd = () => import('../routes/api/v1/admin/auth/logout.post.mjs');
const _lazy_bduzXC = () => import('../routes/api/v1/admin/auth/me.get.mjs');
const _lazy_S2R4L2 = () => import('../routes/api/v1/admin/cancel-requests.get.mjs');
const _lazy_LWIyUu = () => import('../routes/api/v1/admin/cancel-requests/_id/review.post.mjs');
const _lazy_W3AtIJ = () => import('../routes/api/v1/admin/dispatch-reviews.get.mjs');
const _lazy_6Z8Iu8 = () => import('../routes/api/v1/admin/dispatch-reviews/_id/review.post.mjs');
const _lazy_DcPo2_ = () => import('../routes/api/v1/admin/masters.get.mjs');
const _lazy_pDdCjn = () => import('../routes/api/v1/admin/masters/_userId/status.post.mjs');
const _lazy_L52DsX = () => import('../routes/api/v1/admin/orders.get.mjs');
const _lazy_AwIK1e = () => import('../routes/api/v1/admin/orders/_orderId/update.post.mjs');
const _lazy_Iqb5FH = () => import('../routes/api/v1/auth/telegram/init.post.mjs');
const _lazy_cDdF98 = () => import('../routes/api/v1/bootstrap.get.mjs');
const _lazy_46r3t0 = () => import('../routes/api/v1/integrations/telegram/webhook.post.mjs');
const _lazy_jgC9qE = () => import('../routes/api/v1/master/dispatches/_dispatchId_.get.mjs');
const _lazy_ypD5yk = () => import('../routes/api/v1/master/dispatches/_dispatchId/claim.post.mjs');
const _lazy_aNhOBs = () => import('../routes/api/v1/master/orders.get.mjs');
const _lazy_QdvHWl = () => import('../routes/api/v1/master/orders/_orderId_.get.mjs');
const _lazy_jC4nHh = () => import('../routes/api/v1/requests/_requestId_.get.mjs');
const _lazy_Z_Nz0i = () => import('../routes/api/v1/requests/_requestId/address.post.mjs');
const _lazy_Vc38dB = () => import('../routes/api/v1/requests/_requestId/cancel.post.mjs');
const _lazy_w0ZWqj = () => import('../routes/api/v1/requests/_requestId/dispatch.post.mjs');
const _lazy_f3yO2x = () => import('../routes/api/v1/requests/_requestId/intake/confirm.post.mjs');
const _lazy_5YGNQ3 = () => import('../routes/api/v1/requests/_requestId/intake/message.post.mjs');
const _lazy_QYxDPe = () => import('../routes/api/v1/requests/_requestId/intake/messages.get.mjs');
const _lazy_bYf2gb = () => import('../routes/api/v1/requests/_requestId/intake/structured.post.mjs');
const _lazy_P_w36B = () => import('../routes/api/v1/index.get.mjs');
const _lazy_1BHVss = () => import('../routes/api/v1/index.post.mjs');
const _lazy_h3gBiw = () => import('../routes/api/v1/service-domains.get.mjs');
const _lazy_MSUM3c = () => import('../routes/api/v1/service-domains/_domainId/issue-tags.get.mjs');
const _lazy_BYdfzf = () => import('../routes/renderer.mjs').then(function (n) { return n.r; });

const handlers = [
  { route: '/api/v1/admin/auth/login', handler: _lazy_DK8o7k, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/admin/auth/logout', handler: _lazy_vmb_cd, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/admin/auth/me', handler: _lazy_bduzXC, lazy: true, middleware: false, method: "get" },
  { route: '/api/v1/admin/cancel-requests', handler: _lazy_S2R4L2, lazy: true, middleware: false, method: "get" },
  { route: '/api/v1/admin/cancel-requests/:id/review', handler: _lazy_LWIyUu, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/admin/dispatch-reviews', handler: _lazy_W3AtIJ, lazy: true, middleware: false, method: "get" },
  { route: '/api/v1/admin/dispatch-reviews/:id/review', handler: _lazy_6Z8Iu8, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/admin/masters', handler: _lazy_DcPo2_, lazy: true, middleware: false, method: "get" },
  { route: '/api/v1/admin/masters/:userId/status', handler: _lazy_pDdCjn, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/admin/orders', handler: _lazy_L52DsX, lazy: true, middleware: false, method: "get" },
  { route: '/api/v1/admin/orders/:orderId/update', handler: _lazy_AwIK1e, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/auth/telegram/init', handler: _lazy_Iqb5FH, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/bootstrap', handler: _lazy_cDdF98, lazy: true, middleware: false, method: "get" },
  { route: '/api/v1/integrations/telegram/webhook', handler: _lazy_46r3t0, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/master/dispatches/:dispatchId', handler: _lazy_jgC9qE, lazy: true, middleware: false, method: "get" },
  { route: '/api/v1/master/dispatches/:dispatchId/claim', handler: _lazy_ypD5yk, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/master/orders', handler: _lazy_aNhOBs, lazy: true, middleware: false, method: "get" },
  { route: '/api/v1/master/orders/:orderId', handler: _lazy_QdvHWl, lazy: true, middleware: false, method: "get" },
  { route: '/api/v1/requests/:requestId', handler: _lazy_jC4nHh, lazy: true, middleware: false, method: "get" },
  { route: '/api/v1/requests/:requestId/address', handler: _lazy_Z_Nz0i, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/requests/:requestId/cancel', handler: _lazy_Vc38dB, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/requests/:requestId/dispatch', handler: _lazy_w0ZWqj, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/requests/:requestId/intake/confirm', handler: _lazy_f3yO2x, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/requests/:requestId/intake/message', handler: _lazy_5YGNQ3, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/requests/:requestId/intake/messages', handler: _lazy_QYxDPe, lazy: true, middleware: false, method: "get" },
  { route: '/api/v1/requests/:requestId/intake/structured', handler: _lazy_bYf2gb, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/requests', handler: _lazy_P_w36B, lazy: true, middleware: false, method: "get" },
  { route: '/api/v1/requests', handler: _lazy_1BHVss, lazy: true, middleware: false, method: "post" },
  { route: '/api/v1/service-domains', handler: _lazy_h3gBiw, lazy: true, middleware: false, method: "get" },
  { route: '/api/v1/service-domains/:domainId/issue-tags', handler: _lazy_MSUM3c, lazy: true, middleware: false, method: "get" },
  { route: '/__nuxt_error', handler: _lazy_BYdfzf, lazy: true, middleware: false, method: undefined },
  { route: '/__nuxt_island/**', handler: _SxA8c9, lazy: false, middleware: false, method: undefined },
  { route: '/api/_nuxt_icon/:collection', handler: _WYTxYx, lazy: false, middleware: false, method: undefined },
  { route: '/_ipx/**', handler: _28eN2n, lazy: false, middleware: false, method: undefined },
  { route: '/**', handler: _lazy_BYdfzf, lazy: true, middleware: false, method: undefined }
];
...
export { $fetch$1 as $, getResponseStatusText as A, getResponseStatus as B, defineRenderHandler as C, publicAssetsURL as D, createError$1 as E, destr as F, getRouteRules as G, joinURL as H, useNitroApp as I, defu as J, klona as K, hasProtocol as L, isScriptProtocol as M, parseQuery as N, defuFn as O, withQuery as P, sanitizeStatusCode as Q, parseURL as R, encodePath as S, decodePath as T, getContext as U, withTrailingSlash as V, withoutTrailingSlash as W, withLeadingSlash as X, baseURL as Y, executeAsync as Z, encodeParam as _, apiError as a, handler as a0, requireAdminSession as b, clearAdminSessionCookie as c, defineEventHandler as d, getRouterParam as e, computeMissingFields as f, getSupabaseAdmin as g, createAndSendDispatch as h, getQuery as i, setSessionCookie as j, requireUserContext as k, getHeader as l, sendTelegramUserMessage as m, requireMasterContext as n, ok as o, hashRequestPayload as p, buildMasterClaimNotificationText as q, readBody as r, setAdminSessionCookie as s, requireOwnedRequest as t, useRuntimeConfig as u, verifyTelegramInitData as v, createPendingDispatchReview as w, buildAdminReviewNotificationText as x, analyzeIntakeMessage as y, buildAssetsURL as z };;globalThis.__timing__.logEnd('Load chunks/nitro/nitro');
