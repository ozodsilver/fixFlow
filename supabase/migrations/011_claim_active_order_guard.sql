...
begin
  ...
  -- Check master does not already have an active order
  select exists (
    select 1
    from order_assignments oa
    join orders o on o.id = oa.order_id
    where oa.master_id = p_master_user_id
  ...
  if v_exists then
    v_resp := jsonb_build_object(
      'ok', false,
      'error', jsonb_build_object('code', 'master.has_active_order', 'message', 'Master already has an active order')
    );
    v_code := 409;
    update idempotency_keys
    set response_code = v_code, response_body = v_resp
